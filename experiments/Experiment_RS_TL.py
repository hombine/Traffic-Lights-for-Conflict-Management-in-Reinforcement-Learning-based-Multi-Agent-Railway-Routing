...
log_filename = f"training_{datetime.now().strftime('%Y%m%d_%H%M%S')}_exp5.log"
...
def load_bottlenecks_from_json(path):
    with open(path, "r") as f:
        data = json.load(f)

    bottlenecks = {}

    for name, b in data["bottlenecks"].items():
        bottlenecks[name] = {
            "cells": {tuple(cell) for cell in b["cells"]},
            "entry_cells": {
                "before": tuple(b["entry_cells"]["before"]),
                "after": tuple(b["entry_cells"]["after"]),
            },
            "reserved_by": None,
            "direction": None,
        }
...
class EpisodeMetricTracker:
    ...
    def reset(self):
        ...
        self.traffic_light_red_steps = 0
        self.traffic_light_green_events = 0
        self.traffic_light_release_events = 0
        self.traffic_light_auto_reserve_events = 0
        self.traffic_light_waiting_by_agent = defaultdict(int)
        self.traffic_light_entry_counts = Counter()
        self.traffic_light_direction_counts = Counter()
    ...
    def add_traffic_light_red(self, agent, pos, traffic_light_id):
        self.traffic_light_red_steps += 1
        self.traffic_light_waiting_by_agent[agent] += 1
        if pos is not None:
            self.traffic_light_entry_counts[(traffic_light_id, pos)] += 1

    def add_traffic_light_green(self, direction, traffic_light_id):
        self.traffic_light_green_events += 1
        self.traffic_light_direction_counts[(traffic_light_id, direction)] += 1

    def add_traffic_light_release(self, traffic_light_id):
        self.traffic_light_release_events += 1

    def add_traffic_light_auto_reserve(self, direction, traffic_light_id):
        self.traffic_light_auto_reserve_events += 1
        self.traffic_light_direction_counts[(traffic_light_id, direction)] += 1
    ...
    def episode_summary(self, experiment, iteration, episode_id, completion_rate, max_agents, elapsed_steps):
        ...
        return {
            ...
            "traffic_light_green_events": self.traffic_light_green_events,
            "traffic_light_release_events": self.traffic_light_release_events,
            "traffic_light_auto_reserve_events": self.traffic_light_auto_reserve_events,
            "mean_traffic_light_wait_per_agent": (
                np.mean(list(self.traffic_light_waiting_by_agent.values()))
                if self.traffic_light_waiting_by_agent else 0.0
            ),
            "num_traffic_light_entries": len(self.traffic_light_entry_counts),
            "most_blocked_entry_red_steps": (
                max(self.traffic_light_entry_counts.values())
                if self.traffic_light_entry_counts else 0
            ),
            "num_traffic_light_directions_used": len(self.traffic_light_direction_counts),
            "dominant_traffic_light_direction": (
                self.traffic_light_direction_counts.most_common(1)[0][0]
                if self.traffic_light_direction_counts else "none"
            ),
            "dominant_traffic_light_direction_count": (
                self.traffic_light_direction_counts.most_common(1)[0][1]
                if self.traffic_light_direction_counts else 0
            ),
        }
    ...
    def traffic_light_entry_summary(self, experiment, iteration, episode_id):
        rows = []

        for (traffic_light_id, pos), count in self.traffic_light_entry_counts.items():
            x, y = pos
            rows.append({
                "experiment": experiment,
                "iteration": iteration,
                ...
                "entry_x": x,
                "entry_y": y,
                "red_step_count": count,
            })

        return rows

    def traffic_light_direction_summary(self, experiment, iteration, episode_id):
        rows = []

        for (traffic_light_id, direction), count in self.traffic_light_direction_counts.items():
            rows.append({
                "experiment": experiment,
                "iteration": iteration,
                "episode_id": episode_id,
                "traffic_light_id": traffic_light_id,
                "direction": direction,
                "green_or_reserve_count": count,
            })

        return rows


# flatland env wrapper
...
class FlatlandRllibEnv(MultiAgentEnv):
    ...
    def __init__(self, config):
        ...
        # metric init
        self.experiment_name = config.get("experiment_name", "exp5_llm_tl")
        ...
        self.completed_traffic_light_entry_summaries = []
        self.completed_traffic_light_direction_summaries = []
        ...
        # add traffic lights
        traffic_light_config_path = config.get(
            "traffic_light_config_path",
            "llm_outputs/traffic_lights_max_k3.json"
        )

        self.bottlenecks = load_bottlenecks_from_json(
            traffic_light_config_path
        )
    ...
    def drain_completed_metrics(self):
        ...
        traffic_light_entry_rows = self.completed_traffic_light_entry_summaries
        traffic_light_direction_rows = self.completed_traffic_light_direction_summaries
        ...
        return episode_rows, location_rows, traffic_light_entry_rows, traffic_light_direction_rows
    
    def validate_bottlenecks_on_rail(self):
        if getattr(self, "_bottlenecks_validated", False):
            return

        for name, bottleneck in self.bottlenecks.items():
            for cell in bottleneck["cells"]:
                assert self.env.rail.get_full_transitions(*cell) != 0, (
                    f"{name}: controlled cell {cell} is not a rail cell."
                )

            for cell in bottleneck["entry_cells"].values():
                assert self.env.rail.get_full_transitions(*cell) != 0, (
                    f"{name}: entry cell {cell} is not a rail cell."
                )

        self._bottlenecks_validated = True
        print("LLM traffic-light configuration successfully validated.")

    def apply_rule_based_traffic_lights(self, actions):
        controlled_actions = actions.copy()

        for bottleneck_name, bottleneck in self.bottlenecks.items():

            cells = bottleneck["cells"]
            entry_cells = bottleneck["entry_cells"]

            trains_inside = []

            for agent_id, agent in enumerate(self.env.agents):
                if agent.position in cells:
                    trains_inside.append(agent_id)

            # release reservation 
            if bottleneck["reserved_by"] is not None:
                reserved_agent = self.env.agents[bottleneck["reserved_by"]]

                if reserved_agent.position not in cells:
                    bottleneck["reserved_by"] = None
                    bottleneck["direction"] = None
                    self.metric_tracker.add_traffic_light_release(bottleneck_name)

            # auto-reserve 
            if bottleneck["reserved_by"] is None and trains_inside:
                bottleneck["reserved_by"] = trains_inside[0]
                agent = self.env.agents[bottleneck["reserved_by"]]

                bottleneck["direction"] = f"{bottleneck_name}_auto_dir_{agent.direction}"

                self.metric_tracker.add_traffic_light_auto_reserve(
            ...
            for agent_id, action in actions.items():
                ...
                at_before_entry = pos == entry_cells["before"]
                at_after_entry = pos == entry_cells["after"]

                if not at_before_entry and not at_after_entry:
                    continue

                wants_to_enter = action != 0

                if not wants_to_enter:
                    continue

                if bottleneck["reserved_by"] is None:
                    bottleneck["reserved_by"] = agent_id

                    if at_before_entry:
                        bottleneck["direction"] = f"{bottleneck_name}_before_to_after"
                    else:
                        bottleneck["direction"] = f"{bottleneck_name}_after_to_before"

                    self.metric_tracker.add_traffic_light_green(
                        bottleneck["direction"],
                        bottleneck_name
                    )

                elif bottleneck["reserved_by"] == agent_id:
                    pass

                else:
                    controlled_actions[agent_id] = 0
                    self.metric_tracker.add_traffic_light_red(agent_id, pos, bottleneck_name)

        return controlled_actions
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.validate_bottlenecks_on_rail()
    ...
    def step(self, action_dict):
        try:
            ...
            flatland_actions = self.apply_rule_based_traffic_lights(flatland_actions)
            ...
            for agent in rewards:
                ...
                node = obs.get(agent, None)
                
                curr_dist = node.dist_min_to_target
                prev_dist = self.prev_dist.get(agent, None)

                progress_term = 0.0
                if (
                        curr_dist is not None and
                        prev_dist is not None and
                        np.isfinite(curr_dist) and
                        np.isfinite(prev_dist)
                ):
                    progress = prev_dist - curr_dist
                    progress_term = 0.05 * progress


                
                self.prev_dist[agent] = curr_dist

                r = base_reward + living_penalty + progress_term + conflict_penalty
            ...
            if dones["__all__"]:
                ...
                traffic_light_entry_rows = self.metric_tracker.traffic_light_entry_summary(
                    experiment=self.experiment_name,
                    iteration=self.current_iteration,
                    episode_id=self.episode_id
                )

                traffic_light_direction_rows = self.metric_tracker.traffic_light_direction_summary(
                    experiment=self.experiment_name,
                    iteration=self.current_iteration,
                    episode_id=self.episode_id
                )
                ...
                self.completed_traffic_light_entry_summaries.extend(traffic_light_entry_rows)
                self.completed_traffic_light_direction_summaries.extend(traffic_light_direction_rows)
...
def plot_training_curves(reward_history, completion_history, save_dir="plots"):
    ...
    plt.savefig(os.path.join(save_dir, "reward_curve_exp5.png"))
    plt.close()
    ...
    plt.savefig(os.path.join(save_dir, "completion_curve_exp5.png"))
    plt.close()
...
if __name__ == "__main__":
    ...
    config = (
        ...
        .environment(
            ...
            "traffic_light_config_path": "llm_outputs/traffic_lights_max_k3.json"}
        )
        .framework("torch")
        .env_runners(
            num_env_runners=1,
    ...
    metrics_dir = "metrics_exp5"
    episode_metrics_path = os.path.join(metrics_dir, "episode_metrics_exp5.csv")
    location_metrics_path = os.path.join(metrics_dir, "location_metrics_exp5.csv")
    iteration_metrics_path = os.path.join(metrics_dir, "iteration_metrics_exp5.csv")
    traffic_light_entry_path = os.path.join(metrics_dir, "traffic_light_entries_exp5.csv")
    traffic_light_direction_path = os.path.join(metrics_dir, "traffic_light_directions_exp5.csv")
    ...
    for i in range(400):
        ...
        episode_rows = []
        location_rows = []
        traffic_light_entry_rows = []
        traffic_light_direction_rows = []

        for ep_rows, loc_rows, tl_entry_rows, tl_direction_rows in worker_outputs:
            episode_rows.extend(ep_rows)
            location_rows.extend(loc_rows)
            traffic_light_entry_rows.extend(tl_entry_rows)
            traffic_light_direction_rows.extend(tl_direction_rows)

        append_rows_to_csv(episode_metrics_path, episode_rows)
        append_rows_to_csv(location_metrics_path, location_rows)
        append_rows_to_csv(traffic_light_entry_path, traffic_light_entry_rows)
        append_rows_to_csv(traffic_light_direction_path, traffic_light_direction_rows)
    ...
    checkpoint_dir = os.path.abspath("exp5")
